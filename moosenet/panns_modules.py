...
class Cnn14(nn.Module):
    def __init__(
        ...
        # classes_num,
        embedding_size=2048,
        # embedding_size=32,  # same as Cnn14_emb32
        # embedding_size=128,  # same as Cnn14_emb128
        # embedding_size=512,  # same as Cnn14_emb512
        batch_norm_mel_feats=64,
        # batch_norm_mel_feats=32, # same as Cnn14_mel32
    ):
        ...
        self.bn0 = nn.BatchNorm2d(batch_norm_mel_feats)
        ...
        self.conv_block6 = ConvBlock(in_channels=1024, out_channels=embedding_size)

        self.embedding_size = embedding_size

        # self.fc1 = nn.Linear(embedding_size, embedding_size, bias=True)
    ...
    def forward(self, input, mixup_lambda=None):
        ...
        Returns (batch_size, step, out_dim)
        """
        ...
        # Note: PANNS paper experimented also with Mixup in time domain - see original code
        ...
        x = F.dropout(x, p=0.2, training=self.training)
        # combine mel dimension into one
        x = torch.mean(x, dim=3)
        x = x.transpose(
            1, 2
        )  # have last dimension (2) to be dim_out - Note that dimension 1 is not really time
        ...
        # Instead of global pooling local pool and frame wise output
        ...
        # # global pooling
        # # clipwise is mean probably the best option
        # clipwise_output = torch.mean(segmentwise_output, dim=1)
        # # Less prefered alteranative is max pooling
        # # (clipwise_output, _) = torch.max(segmentwise_output, dim=1)
        # # interesting option for explanation is attantion
